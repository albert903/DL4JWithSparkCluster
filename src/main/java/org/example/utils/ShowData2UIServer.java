...
public class ShowData2UIServer {
    ...
    public static void main(String[] args) throws Exception{
        ...
        if (hdfsPath) {
            ...
            InputStream in = fs.open(new Path("/user/root/trainlog/AnimalModelByHdfsTrainingStatsSpark2.dl4j"));
            IOUtils.copyBytes(in, os, 4096, false); //复制到标准输出流
            IOUtils.closeStream(in);
            CommonUtils.closeHdfsConnect(fs);
            os.close();
        } else {
            statsFile = new File("/root/AnimalModelByHdfsTrainingStats1.dl4j");
        }
